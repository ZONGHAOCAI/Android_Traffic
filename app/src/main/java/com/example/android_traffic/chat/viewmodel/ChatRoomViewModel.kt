...
class ChatRoomViewModel : ViewModel() {
    private val url = "http://10.0.2.2:8080/javaweb-Traffic/Chat/ChatRoomController"
...
//    val chatroom: MutableLiveData<ChatRoom> by lazy { MutableLiveData<ChatRoom>() }
    ...
    fun init() {
        ...
        list.value =
            requestTask<List<ChatRoom>>("$url/${member.value!!}", respBodyType = type)
    ...
    fun getNewChatRoom() {
        viewModelScope.launch {
            while (isActive) {
                val type = object : TypeToken<List<ChatRoom>>() {}.type
                val chatroom = requestTask<List<ChatRoom>>("$url/${member.value!!}", respBodyType = type)
